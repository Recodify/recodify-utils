INSERT INTO system.table_definitions
(
  database,
  table,
  category,
...
'Classified logs of all api requests made to lagoon and the queries they produced',
    ...
    $create$,
   [(
     'Populate', 'Load request logs from classfified jsonl', $populate$ clickhouse-client   --host DCDLVLGNCH01   --user user   --password password   --port 9006   --query INSERT INTO sunbath.lagoon_query_classification SETTINGS date_time_input_format='best_effort' FORMAT JSONEachRow" < ./classify/20260317.jsonl $populate$
   )]
)
