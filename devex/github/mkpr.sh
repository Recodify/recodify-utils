mkpr_usage() {
    echo "Recodify mkpr - Make Pull Request - https://github.com/recodify/recodify-utils"
    echo ""
    echo "Usage: mkpr [base_branch] [head_branch]"
    echo "       source /path/to/mkpr.sh"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  mkpr                    # Create PR from current branch to default branch"
    echo "  mkpr develop            # Create PR from current branch to develop"
    echo "  mkpr main feature/foo   # Create PR from feature/foo to main"
    echo ""
    echo "Note: This script can be sourced in .bashrc or run directly"
    echo "      When sourced, it provides the 'mkpr' function globally"
}
...
mkpr() {
  ...
  # Optional overrides: mkpr <base> <head>
  [[ -n "$1" ]] && base="$1"
  [[ -n "$2" ]] && branch="$2"
