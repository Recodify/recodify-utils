...
# Usage: catf [n] [ls options] [pattern...]
# Examples:
#   catf *.txt          # cat the 1st .txt file
#   catf 3 *.json       # cat the 3rd .json file
#   catf 2 -t *.log     # cat the 2nd newest .log file (ls -t)
catf() {
    if [[ "$1" == "-h" || "$1" == "--help" ]]; then
        echo "Usage: catf [n] [ls options] [pattern...]"
        echo "Cat the Nth file from ls output (default n=1)."
        echo ""
        echo "Examples:"
        echo "  catf *.txt          # cat the 1st .txt file"
        echo "  catf 3 *.json       # cat the 3rd .json file"
        echo "  catf 2 -t *.log     # cat the 2nd newest .log file"
        return 0
    fi
    ...
    local file
    file=$(ls "$@" 2>/dev/null | sed -n "${n}p")

    if [[ -z "$file" ]]; then
        echo "Error: no file found at position $n" >&2
        return 1
    elif [[ ! -f "$file" ]]; then
...
# headf - head the first F files (via ls order), L lines each
# Usage: headf [files=1] [lines=10] [ls options] [pattern...]
# Examples:
...
# Examples:
#   tailf 3 50 *.log        # first 3 log files, last 50 lines each
#   tailf 2 20 -t *.csv     # 2 newest csv files, last 20 lines each
tailf() {
  if [[ "$1" == "-h" || "$1" == "--help" ]]; then
    echo "Usage: tailf [files=1] [lines=10] [ls options] [pattern...]"
    echo "Tail the first 'files' files from ls output, printing 'lines' lines per file."
    return 0
  fi
...
rsyncn() {
  local N="${1:?usage: rsync_n N SRC DST}"
  local src="${2:?usage: rsync_n N SRC DST}"
  local dst="${3:?usage: rsync_n N SRC DST}"

  # basic sanity
  [[ "$N" =~ ^[0-9]+$ ]] || { echo "N must be an integer, got: $N" >&2; return 2; }
  [[ -d "$src" ]] || { echo "SRC is not a directory: $src" >&2; return 2; }

  mkdir -p "$dst" || return 1

  find "$src" -maxdepth 1 -type f -printf '%P\0' \
    | sort -z \
    | head -z -n "$N" \
    | rsync -0a --files-from=- "$src"/ "$dst"/
}
